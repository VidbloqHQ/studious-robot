...
const BaseCallControls = ({
  ...
  const updateTrackStates = useCallback(() => {
    ...
    if (newScreenShareState !== isScreenSharingEnabled) {
      ...
      console.log(`Screen share enabled state updated to: ${newScreenShareState}`);
    }
  ...
  const toggleMic = useCallback(() => {
    const publication = p.localParticipant?.getTrackPublication(Track.Source.Microphone);
    if (publication) {
      console.log(`Toggle mic from ${publication.isMuted ? 'muted' : 'unmuted'} to ${publication.isMuted ? 'unmuted' : 'muted'}`);
  ...
  const toggleCamera = useCallback(() => {
    const publication = p.localParticipant?.getTrackPublication(Track.Source.Camera);
    if (publication) {
      console.log(`Toggle camera from ${publication.isMuted ? 'muted' : 'unmuted'} to ${publication.isMuted ? 'unmuted' : 'muted'}`);
  ...
  const toggleScreenShare = useCallback(() => {
    const publication = p.localParticipant?.getTrackPublication(Track.Source.ScreenShare);
    if (publication) {
      console.log(`Toggle screen share from ${publication.isMuted ? 'muted' : 'unmuted'} to ${publication.isMuted ? 'unmuted' : 'muted'}`);
