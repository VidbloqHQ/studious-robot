 ...
 */
export function useMeeting(config?: {
  maxVisibleCoHosts?: Record<string, number>;
}) {
  ...
  const getMaxVisibleCoHosts = () => {
     if (config?.maxVisibleCoHosts?.[screenSize]) {
      return config.maxVisibleCoHosts[screenSize];
    }
    if (screenSize === "xs") return 3;
  ...
  return {
    ...
    overflowTracks,
    screenSize,
    getMaxVisibleCoHosts  // Export the function
  };
}